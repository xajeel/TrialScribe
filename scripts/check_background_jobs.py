#!/usr/bin/env python3
"""Run background-job checks against isolated PostgreSQL, Redis, and Kafka."""
...
INTEGRATION_TESTS = ("services/worker-service/tests/test_jobs_integration.py",)
COORDINATOR_PROBE_GROUP = "trialscribe-jobs-coordinator-readiness"
COORDINATOR_ATTEMPTS = 30
COORDINATOR_RETRY_SECONDS = 2.0


class BackgroundJobCheckFailure(RuntimeError):
    """A safe-to-display background-job integration failure."""
...
class ComposeProject:
    ...
    def capture(self, *arguments: str) -> str:
        ...
        if result.returncode != 0:
            raise BackgroundJobCheckFailure("Docker Compose command failed")
...
def container_value(project: ComposeProject, service: str, key: str) -> str:
    ...
    if not value:
        raise BackgroundJobCheckFailure(f"{service} is missing required configuration")
    return value
...
def published_port(project: ComposeProject, service: str, container_port: str) -> str:
    ...
    if ":" not in published:
        raise BackgroundJobCheckFailure(f"{service} loopback port was not published")
...
def wait_for_group_coordinator(project: ComposeProject) -> None:
    ...
    coordinator is live, so retrying it is the readiness gate (bug B24).
    """
    ...
    raise BackgroundJobCheckFailure("Kafka group coordinator did not become available")
...
def run_command(
    ...
    if result.returncode != 0:
        raise BackgroundJobCheckFailure(output or failure_message)
    if output:
        print(output)
...
def migrate(environment: dict[str, str], redactions: tuple[str, ...]) -> None:
    run_command(
        ...
        environment,
        redactions,
        "Background job migrations failed",
    )
...
def run_integration_tests(environment: dict[str, str], redactions: tuple[str, ...]) -> None:
    run_command(
        ...
        environment,
        redactions,
        "Background job integration tests failed",
    )
...
def run_checks(project: ComposeProject) -> None:
    ...
    environment.update(
        ...
        TRIALSCRIBE_JOBS_INTEGRATION="1",
    )
    ...
    if remaining:
        raise BackgroundJobCheckFailure("Background job checks left an exited container")
...
def main() -> int:
    ...
    try:
        run_checks(project)
    except BackgroundJobCheckFailure as failure:
        print(str(failure), file=sys.stderr)
        return 1
    print("healthy background job queue, progress, retry, cancel, and restart paths")
    return 0
